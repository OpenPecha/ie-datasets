...
def get_token_str(sentences, tokens):
    out = []
    for sentence in sentences:
        start, end, l = sentence['start'], sentence['end'], sentence['len']
        string = str(l) + ''.join([t.content for t in tokens[start:end + 1]])
        out.append(string)
    return out
...
def process(tokens):
    ...
    previous_end = 0
    sentences = extract_chunks(is_endpart_n_shad, tokens, 0, previous_end)
    ...
    # 3.
    s = get_token_str(sentences, tokens)
    # print('ok')
    return s
